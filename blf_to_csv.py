...
def process_blf(input_path, output_path, normalize):
    try:
        script_dir = os.path.dirname(os.path.realpath(__file__))
        ...
        for key, data_points in mat.items():
            if "__" not in key:
                ...
                possible_min = data_points[0][0]
                possible_max = data_points[0][-1]
                if possible_min < min_epoch_utc:
                    min_epoch_utc = possible_min
                if possible_max > max_epoch_utc:
                    max_epoch_utc = possible_max
        ...
        # normalize data by interpreting values
        if normalize:
            for key, data_points in mat.items():
                if "__" not in key:
...
def process_files(checkbox_var):
    ...
    timestamp = process_blf(input_path, output_path, checkbox_var)
...
# Create a label and entry for checkbox
checkbox_var = tk.IntVar()
checkbox = tk.Checkbutton(root, text="Normalize (interpolate) variable timestamps", variable=checkbox_var)
checkbox.grid(row=3, column=0, columnspan=2)

# Create process button
process_button = tk.Button(root, text="Process", command=lambda: process_files(checkbox_var))
process_button.grid(row=4, column=1, padx=5, pady=10)

# Create message label
message_label = tk.Label(root, text="", fg="black")
message_label.grid(row=5, column=1, padx=5, pady=5)
