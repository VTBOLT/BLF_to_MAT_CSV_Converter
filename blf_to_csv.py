...
def interpolate_to_1ms(data_dict):
    '''Function to interpolate data to 1ms steps'''
    interpolated_data = {}
    for key, values in data_dict.items():
        # Extract time and values, ensuring they are floats
        time = np.array([to_float(item[0])
                        for item in values], dtype=np.float64)
        values = np.array([to_float(item[1])
                          for item in values], dtype=np.float64)

        # Create new time array with 1ms steps
        new_time = np.arange(0, time[-1], 0.001)

        # Interpolate values
        new_values = np.interp(new_time, time, values)

        # Store interpolated data
        interpolated_data[key] = list(zip(new_time, new_values))
    ...
    '''
    result_dict = {}
    result_dict["time"] = [item[0]
                                 for item in list(interpolated_data.values())[0]]

    # Add each column header and its interpolated values to the result dictionary
    for key, values in interpolated_data.items():
        result_dict[key] = [item[1] for item in values]
...
def process_blf(input_path, output_path):
    try:
        ...
        interpolate_to_1ms(log_data_seconds)
